...
def ingest_reddit_signals(db, target_date: Optional[date] = None) -> int:
    """
    Ingest Reddit signals from matched events in trends_raw_events.
    Uses events_to_signals aggregation (proper pipeline).
    ...
    logger.info(f"trends_ingest_reddit_start date={target_date} (using events_to_signals)")
    
    try:
        # Use events_to_signals aggregation (proper pipeline)
        from apps.worker.tasks.events_to_signals import aggregate_events_to_signals
        result = aggregate_events_to_signals(db, source="reddit", target_date=target_date)
        signals_inserted = result.get("signals_inserted", 0)
...
def ingest_youtube_signals(db, target_date: Optional[date] = None) -> int:
    """
    Ingest YouTube signals from matched events in trends_raw_events.
    Uses events_to_signals aggregation (proper pipeline).
    ...
    logger.info(f"trends_ingest_youtube_start date={target_date} (using events_to_signals)")
    
    try:
        # Use events_to_signals aggregation (proper pipeline)
        from apps.worker.tasks.events_to_signals import aggregate_events_to_signals
        result = aggregate_events_to_signals(db, source="youtube", target_date=target_date)
        signals_inserted = result.get("signals_inserted", 0)
